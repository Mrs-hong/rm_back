...
namespace qifeng {
    ...
    bool DiwenProtocol::Decode(std::vector<uint8_t>& buffer, Frame& out) {
        ...
        if (headerPos == std::string::npos) {
            // 无完整帧头：只保留末尾可能是"跨读边界帧首"的 0x5A，其余脏字节丢弃；
            // 若整段清空会把下一帧的帧头首字节一并丢掉，导致该帧永久丢失
            if (buffer.back() == mHeaderByte0) {
                buffer.assign(1, mHeaderByte0);
            } else {
                buffer.clear();
            }
            return false;
        ...
        // 最小合法长度 = 命令 + 地址 = 3：再小则地址字段越界、数据区间非法
        constexpr size_t kMinFrameLength = 3;
        if (length < kMinFrameLength) {
            SLOG_WARN << "DiwenProtocol: invalid frame length " << static_cast<int>(length) << ", resync";
            buffer.erase(buffer.begin(), buffer.begin() + 2);  // 丢弃该帧头，重新同步
            return false;
        }
