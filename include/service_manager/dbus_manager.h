...
namespace qifeng::scm {
    ...
    public:
        ...
        /**
         * @brief 获取服务执行结果（success/exit-code/signal/timeout/start-limit-hit等）
         * @param serviceName 服务名称（不含.service后缀）
         * @return ResultMsg 成功时msg为Result值
         */
        ResultMsg GetServiceResult(const std::string &serviceName);

        /**
         * @brief 获取服务主进程退出码
         * @param serviceName 服务名称（不含.service后缀）
         * @return ResultMsg 成功时msg为退出码字符串
         */
        ResultMsg GetServiceExecMainCode(const std::string &serviceName);

        /**
         * @brief 获取服务主进程退出状态
         * @param serviceName 服务名称（不含.service后缀）
         * @return ResultMsg 成功时msg为退出状态字符串
         */
        ResultMsg GetServiceExecMainStatus(const std::string &serviceName);

        /**
         * @brief 获取服务状态描述文本
         * @param serviceName 服务名称（不含.service后缀）
         * @return ResultMsg 成功时msg为StatusText值
         */
        ResultMsg GetServiceStatusText(const std::string &serviceName);
    ...
    private:
        ...
        /**
         * @brief 读取int32类型的DBus属性
         * @param objectPath DBus对象路径
         * @param interface DBus接口名
         * @param property 属性名
         * @return ResultMsg 成功时msg为属性值字符串
         */
        ResultMsg GetInt32Property(const std::string &objectPath, const std::string &interface,
                                   const std::string &property);
