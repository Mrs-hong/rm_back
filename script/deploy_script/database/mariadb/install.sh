...
# 检查 MariaDB 服务端是否已安装且可用
# 仅检查客户端命令（mariadb）不够准确，客户端可能独立于服务端安装
# 需同时验证服务端二进制存在且 dpkg 包状态为"已安装"（ii），确保幂等性
db_log_info "检查 MariaDB 是否已安装..."
if (db_command_exists mariadbd || db_command_exists mysqld) && \
   dpkg -l 2>/dev/null | grep -qE '^ii\s+(mariadb-server|mysql-server)'; then
