...
# 检查端口占用
# 若端口被 MariaDB/MySQL 自身进程占用（install.sh 安装后 apt 自动启动），自动停止服务后继续
# 若端口被其他进程占用，则报错退出
if db_check_port_in_use "$PORT"; then
    PORT_IS_MYSQL=false
    MYSQL_PID=""
    # 方法1：通过 ss 检查监听进程名称（需要 root 权限才能显示进程信息）
    if command -v ss >/dev/null 2>&1; then
        ss -tlnp 2>/dev/null | grep -E ":${PORT}(\s|$)" | grep -qE '"(mysqld|mariadbd)"' && PORT_IS_MYSQL=true
    fi
    # 方法2：通过 lsof + ps 检查监听进程名称（ss 不可用或未显示进程信息时的备选）
    if [[ "$PORT_IS_MYSQL" == "false" ]] && command -v lsof >/dev/null 2>&1; then
        _pid=$(lsof -i :"$PORT" -sTCP:LISTEN -t 2>/dev/null | head -1)
        if [[ -n "$_pid" ]]; then
            _pname=$(ps -p "$_pid" -o comm= 2>/dev/null)
            [[ "$_pname" == "mysqld" || "$_pname" == "mariadbd" ]] && PORT_IS_MYSQL=true
        fi
    fi

    if [[ "$PORT_IS_MYSQL" == "true" ]]; then
        db_log_info "端口 $PORT 已被 MariaDB 服务占用，停止服务后继续初始化..."
        ...
        # 等待端口释放
        _wait=0
        while db_check_port_in_use "$PORT" && [[ $_wait -lt 10 ]]; do
            sleep 1
            ((_wait++))
        done

        # systemd 停止失败时，直接 kill 占用端口的 mysqld/mariadbd 进程
        if db_check_port_in_use "$PORT"; then
            db_log_warn "systemd 停止服务失败，尝试直接 kill 占用端口 $PORT 的 MySQL 进程..."
            # 获取占用端口的 PID
            if command -v lsof >/dev/null 2>&1; then
                MYSQL_PID=$(lsof -i :"$PORT" -sTCP:LISTEN -t 2>/dev/null | head -1)
            elif command -v fuser >/dev/null 2>&1; then
                MYSQL_PID=$(fuser "${PORT}/tcp" 2>/dev/null | tr -d ' ')
            elif command -v ss >/dev/null 2>&1; then
                MYSQL_PID=$(ss -tlnp 2>/dev/null | grep -E ":${PORT}(\s|$)" | sed -n 's/.*pid=\([0-9]\+\).*/\1/p' | head -1)
            fi

            if [[ -n "$MYSQL_PID" ]]; then
                db_log_info "kill 进程 PID=$MYSQL_PID ..."
                kill "$MYSQL_PID" 2>/dev/null || kill -9 "$MYSQL_PID" 2>/dev/null || true
                # 等待端口释放
                _wait=0
                while db_check_port_in_use "$PORT" && [[ $_wait -lt 10 ]]; do
                    sleep 1
            ...
            if db_check_port_in_use "$PORT"; then
                db_exit_with_error $DB_EXIT_PORT_IN_USE "停止 MariaDB 服务后端口 $PORT 仍被占用，无法释放"
            fi
            db_log_info "端口 $PORT 已释放"
        fi
    else
        db_exit_with_error $DB_EXIT_PORT_IN_USE "端口 $PORT 已被其他进程占用"
    fi
fi
...
# 确保数据目录存在并设置正确权限
mkdir -p "$DATA_DIR"
if ! chown -R mysql:mysql "$DATA_DIR"; then
    db_log_error "无法修改数据目录属主为 mysql:mysql。请检查文件系统是否支持 Linux 权限（如 NTFS 不支持）或 mysql 用户是否存在。"
    exit $DB_EXIT_INIT_FAILED
fi
...
EOF
    ...
    # 配置 AppArmor 以允许访问自定义数据目录
    # Ubuntu 默认的 MariaDB AppArmor 策略仅允许访问 /var/lib/mysql/
    # 更换 datadir 后需追加新路径，否则服务启动后会被 AppArmor 拒绝访问而崩溃
    # 同时需为每一级父目录添加 r 权限，否则 AppArmor 路径解析无法穿越
    APPARMOR_PROFILE="/etc/apparmor.d/usr.sbin.mysqld"
    APPARMOR_LOCAL="/etc/apparmor.d/local/usr.sbin.mysqld"
    if [[ -f "$APPARMOR_PROFILE" ]] || [[ -f "$APPARMOR_LOCAL" ]]; then
        # 检查是否已包含自定义路径（幂等性）
        if ! grep -qE "^\s*${DATA_DIR}/" "$APPARMOR_LOCAL" 2>/dev/null; then
            db_log_info "配置 AppArmor 以允许访问自定义数据目录..."
            mkdir -p "$(dirname "$APPARMOR_LOCAL")"

            # 动态生成每一级父目录的 r 权限（路径穿越）+ 数据目录的完整权限
            _aa_rules=""
            _aa_path="/"
            IFS='/' read -ra _aa_parts <<< "${DATA_DIR#/}"
            for _aa_part in "${_aa_parts[@]}"; do
                _aa_path="${_aa_path}${_aa_part}/"
                _aa_rules="${_aa_rules}${_aa_path} r,
"
            done
            _aa_rules="${_aa_rules}${DATA_DIR}/** rwk,
"

            {
                echo ""
                echo "# 由 qifeng-scm init.sh 自动添加，允许 MariaDB 访问自定义数据目录"
                printf '%s' "$_aa_rules"
            } >> "$APPARMOR_LOCAL"
            unset _aa_rules _aa_path _aa_parts _aa_part

            # 重新加载 AppArmor 策略使其生效
            if db_command_exists apparmor_parser; then
...
# 清理残留的运行时文件，避免上次异常退出后 .pid/.sock 残留导致服务启动冲突
rm -f "$DATA_DIR"/*.pid "$DATA_DIR"/*.sock "$DATA_DIR"/*.sock.lock 2>/dev/null || true

if db_command_exists systemctl; then
    systemctl start mariadb 2>/dev/null || systemctl start mysql 2>/dev/null || true
    # systemctl start 可能返回 0 但服务立即崩溃，需等待后检查实际运行状态
    sleep 3
    if systemctl is-active --quiet mariadb 2>/dev/null || systemctl is-active --quiet mysql 2>/dev/null; then
        db_log_info "MariaDB 服务已启动"
    else
        # 服务未存活，输出诊断信息辅助排查
        db_log_error "MariaDB 服务启动失败，诊断信息："
        db_log_error "--- systemd 服务状态 ---"
        systemctl status mariadb 2>/dev/null || systemctl status mysql 2>/dev/null || true
        db_log_error "--- systemd 服务日志 ---"
        journalctl -u mariadb -n 20 --no-pager 2>/dev/null || journalctl -u mysql -n 20 --no-pager 2>/dev/null || true
        db_log_error "--- MariaDB 错误日志 ---"
        if [[ -f "$DATA_DIR"/*.err ]]; then
            tail -n 30 "$DATA_DIR"/*.err 2>/dev/null || true
...
if ! db_wait_for_mysql 30 1; then
    # 超时时输出诊断信息，避免无报错直接退出
    db_log_error "等待 MariaDB 服务就绪超时，诊断信息："
    db_log_error "--- systemd 服务状态 ---"
    systemctl status mariadb 2>/dev/null || systemctl status mysql 2>/dev/null || true
    db_log_error "--- MariaDB 错误日志 ---"
    if [[ -f "$DATA_DIR"/*.err ]]; then
