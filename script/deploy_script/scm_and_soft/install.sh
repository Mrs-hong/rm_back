...
# =============================================================================
# 说明：
#   安装 qifeng-scm deb 包，安装模型 model_qifeng_ca，初始化 nginx 配置，
#   再按 install_order.txt 逐项安装并可选启动服务。
# =============================================================================
...
# [5/8] 查找并安装 SCM deb 包
# ---------------------------------------------------------------------------
log_info "===== [5/8] 安装 SCM deb 包 ====="

deb_files=("${SCRIPT_DIR}"/qifeng-scm*.deb)

if [[ ${#deb_files[@]} -eq 0 ]] || [[ ! -f "${deb_files[0]}" ]]; then
    log_error "未找到 qifeng-scm-*.deb 安装包，请确认文件位于: ${SCRIPT_DIR}"
    exit 1
fi
...
# ---------------------------------------------------------------------------
# [5/8] scm安装模型 model_qifeng_ca 等
# ---------------------------------------------------------------------------
MODEL_DIR="${SCRIPT_DIR}/model_qifeng_ca"
if [[ ! -e "${MODEL_DIR}" ]]; then
    log_error "模型源不存在: ${MODEL_DIR}"
    exit 1
fi

log_info "[5/8] 正在安装模型: model_qifeng_ca ..."
if ! qf_scmc add_model -d "${MODEL_DIR}"; then
    log_error "安装模型 model_qifeng_ca 失败"
    exit 1
fi
log_info "[5/8] 模型 model_qifeng_ca 安装成功"

# ---------------------------------------------------------------------------
# [6/8] 初始化 nginx 配置
# ---------------------------------------------------------------------------
NGINX_INIT_DIR="${SCRIPT_DIR}/nginx_init"
if [[ ! -e "${NGINX_INIT_DIR}" ]]; then
    log_error "nginx 初始化配置源不存在: ${NGINX_INIT_DIR}"
    exit 1
fi

log_info "===== [6/8] 初始化 nginx 配置 ====="
if ! qf_scmc init_nginx -d "${NGINX_INIT_DIR}"; then
    log_error "初始化 nginx 配置失败"
    exit 1
fi
log_info "[6/8] nginx 配置初始化成功"

# ---------------------------------------------------------------------------
# [7/8] 按 install_order.txt 逐项安装服务并验证状态
# ---------------------------------------------------------------------------
log_info "===== [7/8] 安装服务 ====="
...
while IFS= read -r line || [[ -n "$line" ]]; do
    ...
    if ! qf_scmc install -n "${svc_name}" -d "${pkg_path}"; then
