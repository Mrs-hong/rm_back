...
# 卸载软件包
# 仅移除元包（mariadb-server）不够，版本化依赖包（如 mariadb-server-10.3）仍会保留
# 需收集所有已安装的 server/client 相关包一并移除，再通过 autoremove 清理其余依赖
db_log_info "卸载 MariaDB 软件包..."

# 收集状态为 "ii"（已安装）的 MariaDB/MySQL 服务端和客户端包（含版本化包）
INSTALLED_PACKAGES=$(dpkg -l 2>/dev/null | grep -E '^ii\s+(mariadb-server|mariadb-client|mysql-server|mysql-client)' | awk '{print $2}')

if [[ -n "$INSTALLED_PACKAGES" ]]; then
    db_log_info "检测到已安装的包: $INSTALLED_PACKAGES"
    if [[ -n "$PURGE" ]]; then
        apt-get remove --purge -y $INSTALLED_PACKAGES || {
            db_exit_with_error $DB_EXIT_GENERIC_ERROR "卸载 MariaDB 软件包失败"
        }
        apt-get autoremove --purge -y 2>/dev/null || apt-get autoremove -y 2>/dev/null || true
    else
    ...
    # 检查是否存在 "rc" 状态（已删除但保留配置）的包
    RC_PACKAGES=$(dpkg -l 2>/dev/null | grep -E '^rc\s+(mariadb-server|mariadb-client|mysql-server|mysql-client)' | awk '{print $2}')
    if [[ -n "$RC_PACKAGES" ]]; then
        if [[ -n "$PURGE" ]]; then
            db_log_info "MariaDB 软件包已卸载但保留配置，执行 purge 清理..."
            apt-get purge -y $RC_PACKAGES 2>/dev/null || true
            apt-get autoremove --purge -y 2>/dev/null || apt-get autoremove -y 2>/dev/null || true
        else
            db_log_info "MariaDB 软件包已卸载（配置文件保留），跳过卸载步骤"
        fi
    else
...
if [[ -n "$PURGE" ]]; then
    ...
    # 清理 init.sh 添加的 AppArmor 本地配置
    APPARMOR_LOCAL="/etc/apparmor.d/local/usr.sbin.mysqld"
    if [[ -f "$APPARMOR_LOCAL" ]] && grep -q "qifeng-scm init.sh" "$APPARMOR_LOCAL" 2>/dev/null; then
        db_log_info "清理 AppArmor 本地配置中的自定义数据目录规则..."
        # 删除由 init.sh 添加的块（从标记行到 EOF 之前的空行）
        sed -i '/# 由 qifeng-scm init.sh 自动添加/,/^$/d' "$APPARMOR_LOCAL" 2>/dev/null || true
        if db_command_exists apparmor_parser && [[ -f /etc/apparmor.d/usr.sbin.mysqld ]]; then
