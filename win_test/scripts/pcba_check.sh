...
# PCBA 硬件自检汇总脚本
#
# 被 qifeng-scm 的 PcbaChecker 调用，用于整合板子提供商的硬件检测脚本。
#
# 退出码协议（PcbaChecker 依赖此协议判定结果）：
#   exit 0 = 全部通过 (PASS)
#   exit 1 = 存在失败项 (FAIL)
#   exit 2 = 跳过 (SKIP)
#
# 输出关键字协议（当 selftest.json 中 pcba.parse_output=true 时）：
#   输出含 PASS 关键字 → checker 记录为通过信息
#   输出含 FAIL 关键字 → checker 记录为失败信息
#   同时包含时 FAIL 优先级更高
#
# 使用方式：
...
#   4. PcbaChecker 通过 selftest.json:pcba.exe_command 调用本脚本。
# ==========================================================================
...
# 脚本输出中建议包含 PASS/FAIL 关键字，便于 PcbaChecker 做输出解析。
VENDOR_SCRIPTS=(
...
# 每个函数返回 0 表示通过，非 0 表示失败。
# 输出中包含 [PASS] 或 [FAIL] 关键字以配合 PcbaChecker 的 parse_output。
...
check_external_disk() {
    ...
    # TODO：如需读写压力测试，可在此添加 fio/dd 等命令
    # 例如：dd if=/dev/zero of=/mnt/external/testfile bs=1M count=100 conv=fdatasync
    return 0
}
