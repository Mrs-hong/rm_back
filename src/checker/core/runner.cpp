...
namespace qifeng::scm {
    ...
    namespace {
        ...
        /**
         * @brief 执行单个 checker 并强制填入 item/severity，捕获所有异常
         */
        CheckResult RunOne(ICheckerBase &c) {
    ...
    std::vector<CheckResult> Runner::RunAll(const std::vector<std::unique_ptr<ICheckerBase>> &checkers,
                                            int perItemTimeoutSec, bool parallel) {
        std::vector<CheckResult> results;
        results.reserve(checkers.size());
        int timeoutSec = perItemTimeoutSec <= 0 ? 5 : perItemTimeoutSec;

        if (!parallel) {
            // 顺序模式：仍保留超时包裹
            for (const auto &c : checkers) {
                auto fut = std::async(std::launch::async, [&] { return RunOne(*c); });
        ...
        for (const auto &c : checkers) {
            futs.emplace_back(std::async(std::launch::async, [&c] { return RunOne(*c); }));
        }
