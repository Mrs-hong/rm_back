...
namespace qifeng::scm {
    ...
    ResultMsg UdsWrapper::Initialize() {
        if (mSocketFd >= 0) {
            Close();
        }

        if (mMode == UdsMode::SERVER) {
            ResultMsg ret = PrepareSocketPath();
            if (ret.code != 0) {
                return ret;
            }
            ...
            if (mSocketFd < 0) {
                return ResultMsg(-1, "UdsWrapper::Initialize socket failed: " + std::string(std::strerror(errno)));
            }
            ...
            if (bind(mSocketFd, reinterpret_cast<struct sockaddr*>(&addr), sizeof(addr)) < 0) {
                close(mSocketFd);
                mSocketFd = -1;
                return ResultMsg(-1, "UdsWrapper::Initialize bind failed: " + std::string(std::strerror(errno)));
            }
            ...
            if (chmod(mSocketPath.c_str(), static_cast<mode_t>(mSocketMode)) < 0) {
                ...
                mSocketFd = -1;
                RemoveSocketFile();
                return ResultMsg(-1, "UdsWrapper::Initialize chmod failed: " + std::string(std::strerror(errno)));
            }
            ...
            if (listen(mSocketFd, mMaxConnections) < 0) {
                ...
                mSocketFd = -1;
                RemoveSocketFile();
                return ResultMsg(-1, "UdsWrapper::Initialize listen failed: " + std::string(std::strerror(errno)));
            }
        } else {
            mSocketFd = socket(AF_UNIX, SOCK_STREAM, 0);
            if (mSocketFd < 0) {
        ...
        return ResultMsg(0, "UdsWrapper::Initialize success");
    }
    ...
    ResultMsg UdsWrapper::Connect() {
        if (mMode != UdsMode::CLIENT || mSocketFd < 0) {
            return ResultMsg(-1, "UdsWrapper::Connect invalid state");
        }

        struct sockaddr_un addr {};
        addr.sun_family = AF_UNIX;
        if (mSocketPath.size() >= sizeof(addr.sun_path)) {
            return ResultMsg(-1, "UdsWrapper::Connect socket path too long: " + mSocketPath);
        }
        std::strncpy(addr.sun_path, mSocketPath.c_str(), sizeof(addr.sun_path) - 1);

        if (connect(mSocketFd, reinterpret_cast<struct sockaddr*>(&addr), sizeof(addr)) < 0) {
            return ResultMsg(-1, "UdsWrapper::Connect connect failed: " + std::string(std::strerror(errno)));
        }

        return ResultMsg(0, "UdsWrapper::Connect success");
    }
    ...
    ResultMsg UdsWrapper::PrepareSocketPath() {
        if (mSocketPath.empty()) {
            return ResultMsg(-1, "socket path is empty");
        }
        struct sockaddr_un addr {};
        if (mSocketPath.size() >= sizeof(addr.sun_path)) {
            return ResultMsg(-1, "socket path too long");
        }

        // 从socket path中提取目录并创建目录
        auto endIndex = mSocketPath.find_last_of('/');
        std::string parentDir;
        if (endIndex == std::string::npos) {
            parentDir = "/";
        } else {
            parentDir = mSocketPath.substr(0, endIndex);
        }
        ResultMsg ret = utils::CreateDirectory(parentDir);
        if (ret.code != 0) {
            return MakeError("creat sock dir error: " + ret.msg);
        }
