...
namespace qifeng::scm {
    ...
    ResultMsg ServiceManager::StartDependentServices(const std::string &serviceName) {
        ...
        for (const auto &name : mServiceSequence.startOrder) {
            ...
            auto stateResult = mDBusManager->GetUnitActiveState(service_utils::ToSystemdUnitName(name));
            if (stateResult.IsDefaultSuccess() && stateResult.msg == "active") {
                continue;
            }

            auto startResult = mDBusManager->StartUnit(service_utils::ToSystemdUnitName(name));
    ...
    ResultMsg ServiceManager::StartService(const std::string &serviceName) {
        ...
        auto result = mDBusManager->StartUnit(service_utils::ToSystemdUnitName(serviceName));
        if (!result.IsDefaultSuccess()) {
        ...
        auto stateResult = mDBusManager->GetUnitActiveState(service_utils::ToSystemdUnitName(serviceName));
    ...
    ResultMsg ServiceManager::StopService(const std::string &serviceName) {
        ...
        for (const auto &depName : dependents) {
            auto depState = mDBusManager->GetUnitActiveState(service_utils::ToSystemdUnitName(depName));
        ...
        auto result = mDBusManager->StopUnit(service_utils::ToSystemdUnitName(serviceName));
        if (!result.IsDefaultSuccess()) {
        ...
        auto pidResult = mDBusManager->GetServiceMainPID(service_utils::ToSystemdUnitName(serviceName));
        ...
        // 等待进程退出后确认最终状态
        std::this_thread::sleep_for(std::chrono::milliseconds(500));
        auto finalCheck = mDBusManager->GetUnitActiveState(service_utils::ToSystemdUnitName(serviceName));
    ...
    ResultMsg ServiceManager::RestartService(const std::string &serviceName) {
        ...
        auto result = mDBusManager->RestartUnit(service_utils::ToSystemdUnitName(serviceName));
        if (!result.IsDefaultSuccess()) {
    ...
    ResultMsg ServiceManager::UninstallService(const std::string &serviceName) {
        ...
        // 禁用开机自启
        auto disableResult = mDBusManager->DisableUnit(service_utils::ToSystemdUnitName(serviceName));
    ...
    ResultMsg ServiceManager::EnableAutoStart(const std::string &serviceName) {
        ...
        auto result = mDBusManager->EnableUnit(service_utils::ToSystemdUnitName(serviceName));
        if (!result.IsDefaultSuccess()) {
    ...
    ResultMsg ServiceManager::DisableAutoStart(const std::string &serviceName) {
        ...
        auto result = mDBusManager->DisableUnit(service_utils::ToSystemdUnitName(serviceName));
        if (!result.IsDefaultSuccess()) {
    ...
    ResultMsg ServiceManager::StartAllAutoStartServices() {
        ...
        for (const auto &svcName : mServiceSequence.startOrder) {
            ...
            auto stateResult = mDBusManager->GetUnitActiveState(service_utils::ToSystemdUnitName(svcName));
            if (stateResult.IsDefaultSuccess() && stateResult.msg == "active") {
    ...
    ServiceContext ServiceManager::GetServiceContext() const {
        // 返回基础依赖部分上下文：configLoader/fileManager/dbusManager
        // 领域服务成员（serviceManager/nginxManager/modelManager/upgradeService/databaseService）
        // 由 ServiceControl::Init() 构造后填充
        ServiceContext ctx;
