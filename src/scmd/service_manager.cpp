...
namespace qifeng::scm {
    ...
    ServiceManager::ServiceManager(std::shared_ptr<ConfigLoader> configLoader)
        : mConfigLoader(std::move(configLoader)), mFileManager(nullptr), mDBusManager(nullptr),
          mServiceSequence{}, mInitialized(false), mSequenceDirty(true) {
        if (!mConfigLoader) {
    ...
    ResultMsg ServiceManager::StopService(const std::string &serviceName) {
        ...
        for (const auto &depName : dependents) {
            ...
            if (depState.IsDefalutSuccess() && depState.msg == "active") {
                SLOG_INFO << "Warning: service " << depName
                          << " depends on " << serviceName << " and is still running";
            }
        }
