...
namespace qifeng::scm {
    ...
    ResultMsg DBusManager::GetServiceResult(const std::string &serviceName) {
        if (!IsConnected()) {
            return MakeError("DBus is not connected");
        }
        ...
        return GetStringProperty(unitPath, SystemdServiceInterface, "Result");
    }

    ResultMsg DBusManager::GetServiceExecMainCode(const std::string &serviceName) {
        if (!IsConnected()) {
            return MakeError("DBus is not connected");
        }
        ...
        return GetInt32Property(unitPath, SystemdServiceInterface, "ExecMainCode");
    }

    ResultMsg DBusManager::GetServiceExecMainStatus(const std::string &serviceName) {
        if (!IsConnected()) {
            return MakeError("DBus is not connected");
        }
        ...
        return GetInt32Property(unitPath, SystemdServiceInterface, "ExecMainStatus");
    }

    ResultMsg DBusManager::GetServiceStatusText(const std::string &serviceName) {
        if (!IsConnected()) {
            return MakeError("DBus is not connected");
        }
        ...
        return GetStringProperty(unitPath, SystemdServiceInterface, "StatusText");
    }
    ...
    ResultMsg DBusManager::GetInt32Property(const std::string &objectPath, const std::string &interface,
        ...
        // 通过sd_bus_get_property读取int32属性
        int ret = sd_bus_get_property(mBus, SystemdDestination, objectPath.c_str(), interface.c_str(), property.c_str(),
                                      &error, &reply, "i");
        ...
        if (ret < 0) {
            sd_bus_message_unref(reply);
            sd_bus_error_free(&error);
            return MakeError("Failed to read property " + property + ": " + std::string(std::strerror(-ret)));
        }
