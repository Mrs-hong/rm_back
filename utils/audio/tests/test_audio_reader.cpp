...
//
// 用法：
//   ./test_audio_reader                  # 使用编译期默认的 UTILS_AUDIO_TEST_FILE
...
using utils::audio::ErrorCode;
using utils::audio::AudioInfo;
using utils::audio::AudioReader;
using utils::audio::ChannelMixMode;
using utils::audio::ContainerFormat;
using utils::audio::IAudioResampler;
using utils::Result;
using utils::audio::SampleFormat;
using utils::audio::ToString;
...
template <typename T> void CheckOk(const Result<T>& r, const char* expr, const char* file, int line)
{
	...
	if (!r.IsOk()) {
		std::cerr << "[FAIL] " << file << ":" << line << "  expected Ok: " << expr
				  ...
				  << utils::audio::ToMessage(r.Code()) << ")" << " msg=" << r.Message() << "\n";
		++g_failures;
	}
}
...
template <typename T>
void CheckFail(const Result<T>& r, ErrorCode expected, const char* expr, const char* file,
			   int line)
{
	++g_checks;
	if (r.IsOk()) {
...
void TestUpsamplingRejected(const std::string& path)
{
	...
	CHECK_FAIL(r, ErrorCode::UnsupportedResampleDirection);
}
...
void TestCorruptedFile(const std::string& path)
{
	...
	std::string tmpPath = "/tmp/utils_audio_truncated.mp3";
...
void TestErrorCodes(const std::string& path)
{
	...
	// 9a：打开不存在的文件 -> FileNotFound
	{
		...
		CHECK_FAIL(r, ErrorCode::FileNotFound);
	}
	...
	// 9b：Open 之前 ReadFrames -> NotOpened
	{
		...
		CHECK_FAIL(r, ErrorCode::NotOpened);
	}
	...
	// 9e：ReadTimeRangeMs 传入 end<=start -> InvalidArgument
	{
		...
		CHECK_FAIL(r, ErrorCode::InvalidArgument);
	}
}
...
void TestDownmixToMonoAverage(const std::string& path)
{
	...
	CHECK_FAIL(badRes, ErrorCode::InvalidArgument);
...
int main(int argc, char** argv)
{
...
#ifdef UTILS_AUDIO_TEST_FILE
	else {
		path = UTILS_AUDIO_TEST_FILE;
	}
#endif
	...
	std::cout << "utils::audio test suite\n";
