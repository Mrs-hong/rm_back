...
Result<void> MiniaudioDecoder::Open(const std::string& filePath)
{
	...
	if (!test) {
		return Result<void>::Fail(ErrorCode::FileNotFound, "File not found: " + filePath);
	}
	test.close();
	...
	if (mDecoder == nullptr) {
		return Result<void>::Fail(ErrorCode::InternalError, "decoder alloc failed");
	}
	...
	if (mr != MA_SUCCESS) {
		...
		if (mr == MA_INVALID_FILE || mr == MA_NOT_IMPLEMENTED) {
			return Result<void>::Fail(ErrorCode::UnknownFormat,
									  "miniaudio did not recognise file format");
		}
		return Result<void>::Fail(ErrorCode::FileOpenFailed,
	...
	if (mr != MA_SUCCESS || nativeFmt == ma_format_unknown || channels == 0 || sampleRate == 0) {
		...
		delete mDecoder;
		mDecoder = nullptr;
		return Result<void>::Fail(ErrorCode::CorruptedFile,
...
Result<void> MiniaudioDecoder::SeekToFrame(std::uint64_t frameIndex)
{
	...
	if (mInfo.TotalFrames > 0 && frameIndex > mInfo.TotalFrames) {
		return Result<void>::Fail(ErrorCode::OutOfRange, "frameIndex > TotalFrames");
	}
