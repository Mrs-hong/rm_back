...
int main() {
    ...
    std::cout << std::endl << "  --- 6.1 文件不存在 ---" << std::endl;
    {
        ...
        ASSERT_EQ(err.Code(), static_cast<int>(utils::docx::ErrorCode::FileNotFound),
    ...
    std::cout << std::endl << "  --- 6.2 空替换映射 ---" << std::endl;
    {
        ...
        ASSERT_TRUE(openErr.IsOk(), "打开模板");

        std::map<std::string, std::string> empty;
        auto result = doc.ReplaceText(empty);
        ASSERT_EQ(result.Code(), static_cast<int>(utils::docx::ErrorCode::InvalidPattern),
    ...
    std::cout << std::endl << "  --- 6.3 无匹配占位符 ---" << std::endl;
    {
        ...
        ASSERT_EQ(result.Code(), static_cast<int>(utils::docx::ErrorCode::NoMatchFound),
                  "无匹配返回 NoMatchFound");
        ASSERT_EQ(result.Value().totalReplaced, 0, "替换次数为0");
        doc.Close();
    }
    ...
    std::cout << std::endl << "  --- 6.4 未 open 就替换 ---" << std::endl;
    {
        ...
        replacements["测试"] = "值";
        auto result = doc.ReplaceText(replacements);
        ASSERT_EQ(result.Code(), static_cast<int>(utils::docx::ErrorCode::NotOpened),
                  "未 open 返回 NotOpened");
    }
    ...
    std::cout << std::endl << "  --- 6.5 未 open 就保存 ---" << std::endl;
    {
        ...
        ASSERT_EQ(err.Code(), static_cast<int>(utils::docx::ErrorCode::NotOpened),
    ...
    std::cout << std::endl << "  --- 6.7 富文本空映射 ---" << std::endl;
    {
        ...
        std::map<std::string, utils::docx::RichReplacement> empty;
