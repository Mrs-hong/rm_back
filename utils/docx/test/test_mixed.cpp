...
int main() {
    ...
    // 纯文本替换项
    utils::docx::RichReplacement plainTopic;
    plainTopic.content = "年度总结大会";
    plainTopic.type = utils::docx::ContentType::Plain;
    replacements["会议主题"] = plainTopic;

    utils::docx::RichReplacement plainHost;
    plainHost.content = "李四";
    plainHost.type = utils::docx::ContentType::Plain;
    replacements["主持人"] = plainHost;

    // Markdown 富文本替换项
    utils::docx::RichReplacement richBody;
    richBody.content = mdContent;
    richBody.type = utils::docx::ContentType::Markdown;
    ...
    ASSERT_TRUE(result.IsOk(), "混合替换成功");
    ASSERT_TRUE(result.Value().totalReplaced >= 2, "至少替换2处 (纯文本+富文本)");
