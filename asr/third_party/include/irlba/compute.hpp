...
using JacobiSVD = Eigen::JacobiSVD<EigenMatrix_, Eigen::ColPivHouseholderQRPreconditioner>;
...
void exact(const Matrix_& matrix, const Eigen::Index requested_number, EigenMatrix_& outU, EigenMatrix_& outV, EigenVector_& outD) {
    ...
    EigenMatrix_ buffer;
    svd.compute(realizer->realize(buffer), Eigen::ComputeThinU | Eigen::ComputeThinV);
