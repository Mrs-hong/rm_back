    ...
    namespace aas {
        ...
        bool VADTask::Execute(const std::shared_ptr<CommonTaskData>& sharedData) {
            ...
            try {
                ...
                // 所有架构统一调 ModifyAudioSample：把上一批保留的尾部追加到当前块前面，
                // 实现 Buffer 累积（VAD 对整个累积 Buffer 做推理，而非每次只处理 1s）
                ...
                // ---- 滑动窗口 Buffer 尾部决策（基于原始 VAD 分片，在重叠切分之前）----
                // 规则：
                //   离线（末块/停止冲刷）   → 全部分片送推理，清空缓存
                //   无分片（纯静音）        → 无输出，保留末尾 3s 作 VAD 上下文
                //   末尾静音 > 3s           → 全部分片完整，全送；保留末尾 3s（纯静音）
                //   Buffer ≥ 30s           → 强制截断全送；从最后分片末尾保留（避免重发已识别音频）
                //   其余（末段可能不完整）  → 最后一段保留在 Buffer 等下批拼接，其余送推理
                const int64_t bufferEndMs = *timeIntervalMs;
                const bool isOffline = (*pushIntervalMs > bufferEndMs);
                const int64_t tailKeepMs = 3000;   // 尾部保留时长上限
                const int64_t maxBufferMs = 30000; // Buffer 累积上限

                std::vector<std::vector<int>> sendSegs;  // 本批送推理的分片
                int64_t cacheStartMs = -1;               // 尾部缓存起点（-1 = 清空缓存）
                if (actualOutput.empty()) {
                    cacheStartMs = std::max<int64_t>(0, bufferEndMs - tailKeepMs);
                } else if (isOffline) {
                    sendSegs = actualOutput;
                } else {
                    const int64_t lastSegStart = actualOutput.back()[0];
                    const int64_t lastSegEnd = actualOutput.back()[1];
                    if (bufferEndMs - lastSegEnd > tailKeepMs || bufferEndMs >= maxBufferMs) {
                        sendSegs = actualOutput;
                        cacheStartMs = std::max(lastSegEnd, bufferEndMs - tailKeepMs);
                    } else {
                        sendSegs.assign(actualOutput.begin(), actualOutput.end() - 1);
                        cacheStartMs = lastSegStart;
                    }
                }

                if (sendSegs.empty()) {
                    // 增长阶段（最后一段尚未完整）或纯静音：本批无输出，跳过下游推理
                    SLOG_DEBUG << "VAD Task: no complete segment this batch, buffer keeps growing";
                    qifeng::aas::AasResult result;
                    sharedData->Set("result", result);
                    sharedData->SetDataTrusted(false);
                    CacheTailAudio(*meeting, *audioSample, *sampleRate, *absoluteStartTimeMs, cacheStartMs,
                                   bufferEndMs);
                    return true;
                }
                ...
                if ((*archType == AasArchType::PARAFORMER) || (*archType == AasArchType::HYBRID)) {
                    // paraformer 路径（含 HYBRID 实时线）：
                    //   ASR 段按模型上限切分并带重叠缓存（第2段起起点回退，解决切断处识别错误）；
                    //   SV 段不带回退，避免重叠区声纹重复投票
                    const int maxSegMs = static_cast<int>(
                        ConfigManager::GetInstance().GetIntWithConstraint("aas", "paraformer_max_segment_ms", 15000));
                    const int overlapMs = static_cast<int>(
                        ConfigManager::GetInstance().GetIntWithConstraint("aas", "paraformer_overlap_ms", 3000));
                    auto asrSegments = ProcessVadSegments(sendSegs, maxSegMs, overlapMs);
                    auto svSegments = ProcessVadSegments(sendSegs, maxSegMs, 0);
                    asrSpeechData = ExtractAsrFeatures(*audioSample, asrSegments, param);
                    svSpeechData = ExtractSvFeatures(*audioSample, svSegments, param);
                } else {
                    // qwen 路径：合并为单块音频（>30s 的切片与合并由 QwenASRTask 内部完成）
                    asrSpeechData = CalculateAsrAudioSegments(*audioSample, sendSegs, param);
                    svSpeechData = ExtractSvFeatures(*audioSample, sendSegs, param);
                }
                // 缓存尾部音频给下批拼接（须在特征提取后执行：UpdateAudioCache 会 move 走音频）
                CacheTailAudio(*meeting, *audioSample, *sampleRate, *absoluteStartTimeMs, cacheStartMs, bufferEndMs);
        ...
        bool ASRPuncTask::Execute(const std::shared_ptr<CommonTaskData>& sharedData) {
            ...
            // paraformer 路径：合并带重叠切分段的文字
            // ProcessVadSegments 切出的相邻段时间上重叠（cur.start < prev.end），
            // 重叠区文字重复，按"头尾最长公共子串"去重合并（对齐设计文档：
            // "合并文字xxxxxxdd[x]yyyyyy 合并头尾重叠符号最多10个字符"）
            if (asrSpeechData->size() > 1) {
                const int mergeMaxChars = static_cast<int>(
                    ConfigManager::GetInstance().GetIntWithConstraint("aas", "paraformer_merge_max_chars", 10));
                std::vector<qifeng::aas::ASRSpeechAndResultData> mergedSegments;
                mergedSegments.reserve(asrSpeechData->size());
                for (auto& segment : *asrSpeechData) {
                    if (!mergedSegments.empty() && segment.startTime < mergedSegments.back().endTime) {
                        // 时间重叠：合并文字，扩展时间范围到两段并集
                        auto& prev = mergedSegments.back();
                        prev.text = MergeOverlapText(prev.text, segment.text, mergeMaxChars);
                        prev.endTime = std::max(prev.endTime, segment.endTime);
                        prev.isFullSegment = segment.isFullSegment;
                    } else {
                        mergedSegments.push_back(std::move(segment));
                    }
                }
                *asrSpeechData = std::move(mergedSegments);
            }
        ...
        // UTF-8 字符串按字符（Unicode 码点）切分为子串列表
        std::vector<std::string> ASRPuncTask::SplitUtf8Chars(const std::string& text) {
        ...
        // 拼接 UTF-8 字符子串列表为字符串
        std::string ASRPuncTask::JoinUtf8Chars(const std::vector<std::string>& chars) {
            std::string text;
        ...
        // 相邻段文字头尾重叠合并：在 prevText 尾部与 curText 头部找最长公共子串
        // （按 UTF-8 字符，最长 maxChars），命中则去重拼接，未命中直接拼接
        std::string ASRPuncTask::MergeOverlapText(const std::string& prevText, const std::string& curText,
                                                  int maxChars) {
            if (prevText.empty() || curText.empty()) {
                return prevText + curText;
            }
            auto prevChars = SplitUtf8Chars(prevText);
            auto curChars = SplitUtf8Chars(curText);
            const int maxOverlap = std::min({maxChars, static_cast<int>(prevChars.size()),
                                             static_cast<int>(curChars.size())});
            // 从最长到最短尝试：prev 尾部 len 个字符 == cur 头部 len 个字符
            for (int len = maxOverlap; len > 0; --len) {
                ...
                if (matched) {
                    // 去掉 prev 尾部重叠部分后拼接 cur 全文
                    std::string merged = JoinUtf8Chars(
                        std::vector<std::string>(prevChars.begin(), prevChars.end() - len));
                    return merged + curText;
                }
            }
            // 无公共子串：直接拼接
            return prevText + curText;
        ...
        bool QwenASRTask::Execute(const std::shared_ptr<CommonTaskData>& sharedData) {
            ...
            std::vector<qifeng::aas::ASRSpeechAndResultData>* asrSpeechData =
                sharedData->Get<std::vector<qifeng::aas::ASRSpeechAndResultData>>("asrSpeechData");
            int* sampleRate = sharedData->Get<int>("sampleRate");
            // 校验asrSpeechData是否为空
            if (!asrSpeechData || !sampleRate || *sampleRate <= 0) {
                SLOG_ERROR << "QwenASR任务执行失败：asrSpeechData或sampleRate无效";
                sharedData->SetDataTrusted(false);
                return false;
            }

            // qwen 模型单次输入上限 30s：超长音频按 chunk+overlap 切片后逐段推理，
            // 时间戳按重叠中点策略合并（对齐 python asr_timestamp_test.py）
            const int64_t chunkMs =
                ConfigManager::GetInstance().GetIntWithConstraint("aas", "qwen_chunk_ms", 30000);
            const int64_t overlapMs =
                ConfigManager::GetInstance().GetIntWithConstraint("aas", "qwen_overlap_ms", 3000);

            for (auto& segment : *asrSpeechData) {
                if (segment.rawAudioData.empty()) {
                    segment.text = "";
                    continue;
                }
                // 1. 带重叠切片：返回 [(chunk音频, chunk相对段起点偏移ms)]
                auto chunks = ChunkAudioWithOverlap(segment.rawAudioData, *sampleRate, chunkMs, overlapMs);
                SLOG_DEBUG << "  QwenASR切片数: " << chunks.size() << " (chunk=" << chunkMs
                           << "ms, overlap=" << overlapMs << "ms)";

                std::vector<std::vector<AasTimestampItem>> perChunkTs;  // 各 chunk 时间戳（相对 chunk 起点）
                std::vector<int64_t> chunkOffsetsMs;                    // 各 chunk 相对段起点的偏移
                std::vector<int64_t> chunkDurationsMs;                  // 各 chunk 实际时长
                std::string totalText;

                // 2. 逐 chunk 推理：文本直接拼接；时间戳相对 chunk 起点
                for (auto& [chunkAudio, offsetMs] : chunks) {
                    std::string text = qifeng::ModelsManager::GetInstance().QwenASRProcess(chunkAudio);
                    auto ts = qifeng::ModelsManager::GetInstance().QwenForcedAlignProcess(chunkAudio, text);
                    perChunkTs.push_back(std::move(ts));
                    chunkOffsetsMs.push_back(offsetMs);
                    chunkDurationsMs.push_back(static_cast<int64_t>(chunkAudio.size()) * 1000 / *sampleRate);
                    totalText += text;
                    SLOG_DEBUG << "  QwenASR chunk @+" << offsetMs << "ms 文字长度=" << text.size();
                }

                // 3. 重叠中点策略合并各 chunk 时间戳（相对段起点），再平移到绝对时间
                segment.text = totalText;
                auto mergedTs = MergeChunkTimestamps(perChunkTs, chunkOffsetsMs, chunkDurationsMs, overlapMs);
                for (auto& t : mergedTs) {
                    t.startMs += segment.startTime;
                    t.endMs += segment.startTime;
                }
                segment.timestamps = std::move(mergedTs);

                SLOG_DEBUG << "  QwenASR实际输出: sequence=" << segment.text
                           << ", timestamps=" << segment.timestamps.size();
            }
        ...
        std::vector<std::pair<std::vector<float>, int64_t>>
        QwenASRTask::ChunkAudioWithOverlap(const std::vector<float>& audio, int sampleRate, int64_t chunkMs,
                                           int64_t overlapMs) {
            std::vector<std::pair<std::vector<float>, int64_t>> chunks;
            if (audio.empty() || sampleRate <= 0 || chunkMs <= 0) {
                return chunks;
            }
            const int64_t chunkLen = chunkMs * sampleRate / 1000;  // chunk 样本数
            int64_t hopLen = chunkLen - overlapMs * sampleRate / 1000;  // 步长 = chunk - overlap
            if (hopLen <= 0) {
                hopLen = chunkLen;  // 防御：overlap >= chunk 时退化为无重叠
            }
            const int64_t total = static_cast<int64_t>(audio.size());
            int64_t pos = 0;
            while (pos < total) {
                const int64_t end = std::min(pos + chunkLen, total);
                chunks.emplace_back(std::vector<float>(audio.begin() + pos, audio.begin() + end),
                                    pos * 1000 / sampleRate);
                if (end >= total) {
                    break;
                }
        ...
        //   与上一段重叠区中点 prevMid = offsets[i] + overlap/2，与下段重叠区中点
        //   nextMid = offsets[i+1] + overlap/2；只保留落在 (prevMid, nextMid) 内的条目，
        //   即前 chunk 取到重叠中点之前、后 chunk 从中点之后开始，避免重叠区重复）
        std::vector<AasTimestampItem>
        QwenASRTask::MergeChunkTimestamps(const std::vector<std::vector<AasTimestampItem>>& perChunkTs,
                                          const std::vector<int64_t>& chunkOffsetsMs,
                                          const std::vector<int64_t>& chunkDurationsMs, int64_t overlapMs) {
            std::vector<AasTimestampItem> merged;
            const size_t n = perChunkTs.size();
            for (size_t i = 0; i < n; ++i) {
                const int64_t chunkStart = chunkOffsetsMs[i];
                const int64_t chunkEnd = chunkStart + chunkDurationsMs[i];
                // 与下一段的重叠中点（最后一段不截断）
                const int64_t nextMid = (i + 1 < n) ? chunkOffsetsMs[i + 1] + overlapMs / 2 : chunkEnd + 1;
                // 与上一段的重叠中点（第一段不截断）
                const int64_t prevMid = (i > 0) ? chunkOffsetsMs[i] + overlapMs / 2 : chunkStart - 1;

                for (const auto& ts : perChunkTs[i]) {
                    // 时间戳相对 chunk 起点，加偏移得相对整段音频的绝对位置
                    const int64_t absStart = ts.startMs + chunkOffsetsMs[i];
                    const int64_t absEnd = ts.endMs + chunkOffsetsMs[i];
                    if (absEnd <= prevMid) {
                        continue;  // 落在上一段重叠区内，已由上一段覆盖
                    }
                    if (absStart >= nextMid) {
                        continue;  // 落在下一段重叠区内，留给下一段覆盖
                    }
                    merged.push_back({ts.text, absStart, absEnd});
                }
            }
            return merged;
        }
        ...
        bool AggregationTask::Execute(const std::shared_ptr<CommonTaskData>& sharedData) {
            ...
            // asr 段与 sv 段数量可能不同：paraformer 路径下 asr 段经重叠切分+文字合并，
            // sv 段无重叠切分（避免声纹重复投票）。因此按"时间重叠最大"原则匹配说话人。
            int64_t* absoluteStartTimeMs = sharedData->Get<int64_t>("absoluteStartTimeMs");
            const int64_t timeOffsetMs = (absoluteStartTimeMs ? *absoluteStartTimeMs : 0);
            qifeng::aas::AasResult result;
            result.code = 0;
            for (size_t i = 0; i < asrSpeechData->size(); ++i) {
                const auto& asrSeg = asrSpeechData->at(i);
                // 找与当前 asr 段时间重叠最大的 sv 段（sv 段时间为相对时间，需加偏移）
                const SVSpeechAndResultData* bestSv = nullptr;
                int64_t bestOverlap = -1;
                for (const auto& svSeg : *svSpeechData) {
                    const int64_t overlap = std::min(asrSeg.endTime, svSeg.endTime + timeOffsetMs) -
                                            std::max(asrSeg.startTime, svSeg.startTime + timeOffsetMs);
                    if (overlap > bestOverlap) {
                        bestOverlap = overlap;
                        bestSv = &svSeg;
                    }
                }
                qifeng::aas::AasSegment seg {.startTime = asrSeg.startTime,
                                             .endTime = asrSeg.endTime,
                                             .text = std::move(asrSeg.text),
                                             .speakerLabel = -1,
                                             .isFullSegment = true};
                if (bestSv != nullptr) {
                    seg.speakerLabel = bestSv->speakerId;
                    seg.speakerName = bestSv->svResult;
                    seg.svEmbedding = bestSv->svEmbedding;
                    seg.svEmbeddingMd5 = bestSv->svEmbeddingMd5;
                    seg.isFullSegment = bestSv->isFullSegment;
                }
                // 透传词级时间戳（qwen 路径输出；paraformer 路径为空）
                seg.timestamps = asrSeg.timestamps;
                seg.asrContent = asrSeg.asrContent;
        ...
        std::vector<std::vector<int>> VADTask::ProcessVadSegments(std::vector<std::vector<int>>& segments,
                                                                  int max_segment_time, int overlap_ms) {
            ...
            // 3. 切分步长 = 上限 - 重叠：第2段起起点回退 overlap_ms，与上一段尾部形成重叠区，
            //    解决切断处的头部/尾部识别错误（对齐设计文档举例：vad分片10~35s →
            //    asr处理10~25s、22~35s，合并重叠文字；SV 场景传 overlap_ms=0 无重叠）
            int step = max_segment_time - overlap_ms;
            if (step <= 0) {
                // 防御：重叠 >= 上限时退化为无重叠切分，避免死循环
                step = max_segment_time;
            }
            ...
            for (const auto& seg : merged) {
                ...
                // 5. 对长分段进行带重叠切分（cur 按步长推进，段长最长 max_segment_time）
        ...
        void VADTask::CacheTailAudio(std::shared_ptr<AasMeeting>& meeting, const std::vector<float>& audioSample,
                                     int sampleRate, int64_t absoluteStartTimeMs, int64_t cacheStartMs,
                                     int64_t bufferEndMs) {
            if (cacheStartMs >= 0 && bufferEndMs > cacheStartMs) {
                auto tailAudio =
                    ExtractAudioSegment(audioSample, sampleRate, static_cast<int>(cacheStartMs),
                                        static_cast<int>(bufferEndMs));
                if (!tailAudio.empty()) {
                    const int64_t remainMs = bufferEndMs - cacheStartMs;
                    meeting->UpdateAudioCache(tailAudio, true,
                                              {cacheStartMs + absoluteStartTimeMs, bufferEndMs + absoluteStartTimeMs,
                                               remainMs});
                    return;
                }
            }
            std::vector<float> emptyAudio;
            meeting->UpdateAudioCache(emptyAudio, false, {0, 0, 0});
        }
        ...
        VADTask::ExtractAsrFeatures(const std::vector<float>& audioSample, std::vector<std::vector<int>>& segments,
            ...
            // paraformer 路径（含 HYBRID 实时线）：对给定分片提取 ASR 特征
            // （滑动窗口的发送/保留决策已在 Execute 中基于原始 VAD 分片完成）
            std::vector<qifeng::aas::ASRSpeechAndResultData> asrSpeechData;
            for (const auto& segment : segments) {
                auto audioSegment = ExtractAudioSegment(audioSample, param.sampleRate, segment[0], segment[1]);
                if (!audioSegment.empty()) {
                    auto res = qifeng::ModelsManager::GetInstance().AsrFeatureProcess(audioSegment);
                    // 聚合初始化按成员声明顺序：rawAudioData, speechData, text, timestamps,
                    // startTime, endTime, isFullSegment
                    asrSpeechData.push_back({{}, res, "", {},
                                             segment[0] + param.absoluteStartTimeMs,
                                             segment[1] + param.absoluteStartTimeMs, 0});
                } else {
                    SLOG_WARN << "  ASR片段无效，跳过: [" << segment[0] + param.absoluteStartTimeMs << ","
                              << segment[1] + param.absoluteStartTimeMs << "]";
                }
            }
        ...
        VADTask::CalculateAsrAudioSegments(const std::vector<float>& audioSample,
            ...
            // qwen 路径：每个 VAD 分片独立提取 rawAudioData，不合并多段
            //   时间戳以分片 startTime/endTime 为基准；单分片超 30s 的二次切片由
            //   QwenASRTask::ChunkAudioWithOverlap 处理。
            //   （滑动窗口的发送/保留决策已在 Execute 中基于原始 VAD 分片完成）
            std::vector<qifeng::aas::ASRSpeechAndResultData> asrSpeechData;
            for (const auto& seg : segments) {
                auto audio = ExtractAudioSegment(audioSample, param.sampleRate, seg[0], seg[1]);
                if (!audio.empty()) {
                    // 聚合初始化按成员声明顺序：rawAudioData, speechData, text, timestamps,
                    // startTime, endTime, isFullSegment
                    asrSpeechData.push_back({audio, {{}}, "", {},
                                             seg[0] + param.absoluteStartTimeMs,
                                             seg[1] + param.absoluteStartTimeMs, 0});
                }
            }
            return asrSpeechData;
        }
