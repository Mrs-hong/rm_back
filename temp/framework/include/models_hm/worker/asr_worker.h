...
namespace qifeng {
    ...
    public:
         ...
         * @param ioMode IO 模式
         */
        ASRWorker(const std::string& encoderPath, const std::string& decoderPath, const std::string& predictorPath,
                  int tpuId = 0, IOMode ioMode = IOMode::SYSIO);
        ~ASRWorker();
    ...
    private:
        ...
        // 初始化ONNX Runtime
        bool InitOnnxRuntime(const std::string& modelPath);
