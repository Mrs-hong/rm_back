...
#ifndef QIFENG_FRAMEWORK_INCLUDE_MODELS_WORKER_ASR_WORKER_H
#define QIFENG_FRAMEWORK_INCLUDE_MODELS_WORKER_ASR_WORKER_H
...
namespace qifeng {
    ...
    /**
     * @class ASRWorker
     * @brief ASR模型工作者类，继承自ModelWorker基类
     * @note 使用std::any作为输入类型，std::string作为输出类型
     */
    class ASRWorker : public ModelWorker {
    public:
        ASRWorker(const std::string& modelPath, int tpuId = 0, IOMode ioMode = IOMode::SYSIO);
        ~ASRWorker();
...
#endif  // QIFENG_FRAMEWORK_INCLUDE_MODELS_WORKER_ASR_WORKER_H