 ...
 * 使用示例（富文本混合替换）：
 * @code
 *   qifeng_ca::docx::DocxDocument doc;
 *   doc.Open("input.docx");
 *   std::map<std::string, qifeng_ca::docx::RichReplacement> rich;
 *   rich["正文"] = {mdContent, qifeng_ca::docx::ContentType::Markdown};
 *   rich["会议主题"] = {"年终总结", qifeng_ca::docx::ContentType::Plain};
...
namespace qifeng_ca::docx {
    ...
    public:
        ...
        /// 富文本批量替换：支持纯文本/HTML/Markdown/EditorJS 混合替换
        /// @param replacements 占位符名称→富文本替换项 的映射
        ///                       RichReplacement.type 决定内容类型：
        ///                         - Plain: 走纯文本替换
        ///                         - HTML: 解析 HTML 并生成格式化段落
        ///                         - Markdown: 解析 MD 并生成格式化段落
        ///                         - EditorJS: 解析 Editor.js JSON 并生成格式化段落
        ///                       HTML/Markdown/EditorJS 仅替换独占一行的占位符（替换整个段落）
        /// @return 替换结果（含错误信息与逐项记录）
        ...
        /// @param contentType 内容类型（Plain/HTML/Markdown/EditorJS）
        /// @return 结果（含错误信息与生成记录）
        ///
        /// @note 需先调用 Open() 打开空白模板，生成后调用 Save() 保存
        /// @note Plain 类型按行分割，每行渲染为仿宋正文段落
        /// @note HTML/Markdown/EditorJS 类型使用富文本解析器生成格式化段落
        ReplaceResult GenerateDocument(const std::string &title, const std::string &bodyContent,
                                       ContentType contentType);

        /// 从空白模板生成文档（传入预构造的段落列表）
        ///
        /// 在已打开的空白模板中插入标题和预构造的段落列表，生成完整文档。
        /// 适用于需要混合多种段落类型（如小标题+会议信息+正文）的场景，
        /// 调用方自行构造 RichParagraph 列表，标题作为 h1 居中渲染。
        ///
        /// @param title 标题（可选，为空则不添加标题）
        /// @param paragraphs 正文段落列表（已包含格式信息）
        /// @return 结果（含错误信息与生成记录）
        ///
        /// @note 需先调用 Open() 打开空白模板，生成后调用 Save() 保存
        /// @note 段落列表为空时返回 InvalidPattern 错误
        /// @note 内部会先插入标题段落(h1), 再追加 paragraphs 中的所有段落
    ...
    private:
        ...
        bool ShouldUseStreaming(size_t extraSize) const;

        /// 解压 docx 到 mTempDir
        ErrorInfo Unzip();

        /// 加载 document.xml 到 mDomDoc
        ErrorInfo ParseXmlDom();

        /// 保存 mDomDoc 到 document.xml
        ErrorInfo SaveXmlDom();

        /// 将 mTempDir 压缩为 docx
        ErrorInfo Zip(const std::string &outputPath);

        /// 创建临时目录
        ErrorInfo CreateTempDir();
        ...
        ReplaceResult ReplaceTextDom(const std::map<std::string, std::string> &replacements);
        ReplaceResult ReplaceRichDom(const std::map<std::string, RichReplacement> &replacements);

        // ── 流式模式替换 ──

        ReplaceResult ReplaceTextStreaming(const std::map<std::string, std::string> &replacements);
        ReplaceResult ReplaceRichStreaming(const std::map<std::string, RichReplacement> &replacements);
    };
