...
/**
 * @file zip_utils.cpp
 * @brief ZIP 工具实现：基于 minizip-ng 向后兼容层的解压/压缩功能
 *
 * minizip-ng 提供 zip.h/unzip.h/ioapi.h 兼容接口, API 与原始 minizip 一致,
 * 仅部分参数类型从 int 调整为 uint16_t/uint32_t/unsigned long(见各处显式转换)。
 */
...
#include <unzip.h>
#include <ioapi.h>
#include <minizip/mz.h>  // MZ_ZIP_FLAG_UTF8
...
bool UnzipToDir(const std::string& zipPath, const std::string& destDir) {
    ...
    do {
        ...
        // 文件名缓冲区较大, 使用堆分配避免超出栈使用上限(-Wstack-usage=5120)
        constexpr int FILENAME_SIZE = 4096;
        std::vector<char> fileNameBuf(FILENAME_SIZE);
        if (unzGetCurrentFileInfo(uf, &fileInfo, fileNameBuf.data(), FILENAME_SIZE,
                                   nullptr, 0, nullptr, 0) != UNZ_OK) {
            success = false;
            break;
        }
        const char* fileName = fileNameBuf.data();
        ...
        // 读取并写入数据(缓冲区较大, 使用堆分配避免超出栈使用上限)
        constexpr int BUFFER_SIZE = 8192;
        std::vector<char> buffer(BUFFER_SIZE);
        int readBytes;
        while ((readBytes = unzReadCurrentFile(uf, buffer.data(), BUFFER_SIZE)) > 0) {
            if (fwrite(buffer.data(), 1, static_cast<size_t>(readBytes), outFile)
...
bool ZipDir(const std::string& srcDir, const std::string& zipPath,
    ...
    bool success = true;
    // 缓冲区较大, 使用堆分配避免超出栈使用上限(-Wstack-usage=5120)
    constexpr int BUFFER_SIZE = 8192;
    std::vector<char> buffer(BUFFER_SIZE);
    ...
    for (const auto& [relPath, absPath] : files) {
        ...
        if (zipOpenNewFileInZip4(zf, relPath.c_str(), &zi,
                                  nullptr, 0, nullptr, 0, nullptr,
                                  method, level, 0, -MAX_WBITS, DEF_MEM_LEVEL,
                                  Z_DEFAULT_STRATEGY, nullptr, 0, 0, MZ_ZIP_FLAG_UTF8) != ZIP_OK) {
            fclose(inFile);
            success = false;
            continue;
        }
        ...
        // 读取文件内容并写入 ZIP
        size_t readBytes;
        while ((readBytes = fread(buffer.data(), 1, BUFFER_SIZE, inFile)) > 0) {
            if (zipWriteInFileInZip(zf, buffer.data(), static_cast<unsigned int>(readBytes)) != ZIP_OK) {
                success = false;
                break;
            }
        }
...
bool ZipSingleFile(const std::string& filePath, const std::string& zipPath,
    ...
    zip_fileinfo zi;
    memset(&zi, 0, sizeof(zi));
    if (zipOpenNewFileInZip4(zf, entryName.c_str(), &zi,
                              nullptr, 0, nullptr, 0, nullptr,
                              method, level, 0, -MAX_WBITS, DEF_MEM_LEVEL,
                              Z_DEFAULT_STRATEGY, nullptr, 0, 0, MZ_ZIP_FLAG_UTF8) != ZIP_OK) {
        fclose(inFile);
